...
<div class="row">
    ...
    <div class="col-sm-6">
        ...
        <h2>
           ${profile['activity_points'] or "No"} ${ _("activity point") }${'s' if profile['activity_points'] != 1 else ''}<br />
           ${profile["number_projects"]} ${ _("project") }${'s' if profile["number_projects"] != 1  else ''}<!-- , ${profile["number_public_projects"]} public -->
        </h2>
        <h2>
            ${ _("Usage of % strage", 'NII' if profile['quota']['is_nii_storage'] else 'Institutional') }<br />
