...
<%def name="title()">${ _("Personal Token Detail") }</%def>
<%def name="content()">
...
<div id="personalTokenDetailPage" class="row">
    ...
    <div class="col-sm-9 col-md-7" id="tokenDetail" style="display:none;" data-bind="visible: true">
        <div class="row">
            <div class="col-sm-12">
                <div class="breadcrumb"><i class="fa fa-angle-double-left"></i> <a data-bind="attr: {href: $root.webListUrl}">${ _("Return to list of registered tokens") }</a></div>
            </div>
        </div>
                ...
                <form novalidate class="form-inline" role="form" data-bind="submit: $root.submit.bind($root), validationOptions: {insertMessages: false, messagesOnModified: false}">
                    ...
                    <div class="padded action-buttons">
                        ...
                        <button type="button" class="btn btn-danger"
                                data-bind="visible: !$root.isCreateView(), click: $root.deleteToken.bind($root)">${ _("Delete") }</button>
                        <button type="submit" class="btn btn-success"
